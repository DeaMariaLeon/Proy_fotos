...
def check_outputhtml(path, DATABASE):
...
try:
    with open("variables_db.json", "r") as json_file:
    ...
    print("Couldn't open variables_db.json file")

if Path(path).exists():
    used_path, new_output, soup, num_ofpics, last_fotito = check_outputhtml(path, database)
